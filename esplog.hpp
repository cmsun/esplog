...
public:
    ...
    // 调试日志（蓝色）
    template <typename... Args> static inline void debug(fmt::format_string<Args...> fmt, Args &&...args)
    {
    ...
    // 信息日志（绿色）
    template <typename... Args> static inline void info(fmt::format_string<Args...> fmt, Args &&...args)
    {
    ...
    // 警告日志（黄色）
    template <typename... Args> static inline void warn(fmt::format_string<Args...> fmt, Args &&...args)
    {
    ...
    // 错误日志（红色）
    template <typename... Args> static inline void error(fmt::format_string<Args...> fmt, Args &&...args)
    {
...
private:
    ...
    static inline std::vector<std::pair<int, sockaddr_in>> _client_info; // 客户端信息
   ...
   */
    static void wifi_init(std::string_view ssid, std::string_view password)
    {
        ...
        size_t pwd_len = std::min<size_t>(password.size(), sizeof(ap_config.ap.password) - 1);
   ...
   */
    static void tcp_server_init(uint16_t port)
    {
        ...
        if (bind(_server_sock, reinterpret_cast<sockaddr *>(&server_addr), sizeof(server_addr)) < 0)
        {
            close(_server_sock);
            _server_sock = -1;
            return;
   ...
   */
    static void accept_task(void *arg)
    {
        while (true)
        {
            ...
            int client_sock = accept(_server_sock, reinterpret_cast<sockaddr *>(&client_addr), &addr_len);
